...
This installs the package in editable mode; `-c constraints.txt` pins the dev dependencies to the exact versions CI uses, so local tool runs match CI's. `pre-commit install` wires the git hook that runs ruff before each commit.
...
| Decide | `cleanrr-planner` → `cleanrr-architect`, on demand | Opus | One hard structural decision at a time (process model, schema, trust boundary, new interface). Returns a short memo the planner copies into the plan. A decision that turns on the owner's money, terms, or product direction is sent back to the owner and blocks `/cleanrr-ship` until answered. |
...
Both skills are meant to be run from a fresh session: every turn re-reads the whole conversation, and an orchestrator carrying unrelated history costs as much as all of its agents. When a run has to start from a long session anyway, `cleanrr-orchestrator` does the same work in its own context with a one-hour prompt cache, because a subagent's default five-minute cache is rewritten in full after every wait on another agent. The reviewer and the security agent run on every change. `/cleanrr-ship light: <request>` skips only the planner, for a docs-only change or one small source file, and `.claude/hooks/lane.py` decides whether a change qualifies; anything it does not recognise takes the full lane. Each run ends with `.claude/hooks/usage_report.py`, which prints turns, tokens, and list-price cost per agent.
...
- `.claude/hooks/plan_lint.py` checks a plan's structure (every task heading, commit subjects, no file owned twice in a wave, task width) as the planner's Stop hook and again before `/cleanrr-ship` starts, so a malformed plan never reaches a coder.
...
- Agent frontmatter allowlists do the rest: coders have no web access (the planner already verified the facts they need), review agents and the architect can't spawn agents or write files, the planner edits only its own plan files.
