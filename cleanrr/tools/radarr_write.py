...
def build_tools(
    ...
    @tool(
        ...
        "Kicks off another search — if Radarr finds a matching release it may grab "
        "it immediately, even if something's already downloading, so this is not a "
        "no-op. The user confirms in chat first. Use when a movie request has been "
        "sitting with no progress and the user wants to nudge it. Pass the title as "
        "the user said it.",
        {"title": str},
    )
    async def force_research_movie(args: dict[str, Any]) -> dict[str, Any]:
        ...
        if error_response is not None:
            metrics.tool_calls_total.labels(tool="force_research_movie", status=lookup.status).inc()
            return error_response

        if lookup.request is None:
            metrics.tool_calls_total.labels(tool="force_research_movie", status="http_error").inc()
            return text_result(
        ...
        if tmdb_id is None or not isinstance(tmdb_id, int):
            metrics.tool_calls_total.labels(tool="force_research_movie", status="not_a_movie").inc()
            return text_result(
        ...
        try:
            ...
            metrics.tool_calls_total.labels(tool="force_research_movie", status="parse_error").inc()
            return text_result(
        ...
        metrics.tool_calls_total.labels(tool="force_research_movie", status="success").inc()
