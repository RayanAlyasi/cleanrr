...
**Container image**: [Trivy](https://github.com/aquasecurity/trivy) scans the built image for CRITICAL and HIGH vulnerabilities on every pull request and push to `main` (`docker.yml`) and again in the release build (`release.yml`), with the same settings. A finding that has a fix available fails the job; findings with no fix available are excluded (`ignore-unfixed`). The pull-request scan is not a required status check.
