...
Single Python process, single container. Tools are defined as in-process `@tool` functions on the Agent SDK — no separate MCP server processes to run. Each Telegram user gets their own dedicated `Agent` (one Claude CLI subprocess), lazily created and capped by `AgentPool` — see [Project layout](#project-layout) below. An Agent is stopped after `AGENT_IDLE_TIMEOUT_MINUTES` with no message from that user (`0` disables it) or immediately on `/reset`; either way, the user's next message starts a fresh subprocess with no memory of the previous conversation, and the stop itself is drained in a background task so neither the Telegram update loop nor another user's turn ever waits on it. Each subprocess runs with user, project and local settings and their `CLAUDE.md` discovery disabled, in a pinned empty working directory, so a settings file in a checkout or a home directory cannot change what the agent is allowed to do; enterprise managed settings are outside this control.
...
├── agent_pool.py      # AgentPool: creates/caps one Agent per telegram_user_id, runs the idle-eviction sweeper
