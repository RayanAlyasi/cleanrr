...
**Residual risk**: low. A linked user or admin can still flood the bot within those caps, and an unlinked stranger can still make the bot emit one refusal reply and one log line per message — Telegram's own flood limits are the only bound on that. A retiring slot is held until that Agent's in-flight turn ends, bounded by `Agent._max_lock_hold_seconds()` (about seven and a half minutes at defaults), plus the ~20s close; one user holds at most two slots — one live, one retiring — and runs at most two turns at once, because `/reset` is refused while their previous one is still stopping. The cap bounds how bad that gets and it clears itself once the stop completes. This is the one category where the codebase already treats DoS as a first-class concern with enforced numeric limits, not just documentation.
