...
4. **Clean up:** for each integrated coder, `git worktree remove --force <worktree-path>` then `git branch -D <coder-branch>` (a squash leaves no ancestry, so `-d` would refuse); finally `git worktree prune`.
